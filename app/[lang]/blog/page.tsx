...
async function getPosts() {
  try {
    const res = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/d1/database/${dbId}/query`, {
      ...
      body: JSON.stringify({ sql: "SELECT p.*, u.username as author_username FROM posts p LEFT JOIN users u ON p.author = u.display_name WHERE p.status = 'published' AND datetime(p.published_at) <= datetime('now') ORDER BY p.created_at DESC" }),
