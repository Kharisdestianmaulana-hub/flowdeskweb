...
export default function ChangelogStrip({ commits, dict, currentLang }: ChangelogStripProps) {
  ...
  return (
    <section className="bg-[var(--color-primary)] py-16 border-t border-white/10">
              ...
              {commits.map((item, index) => (
                <a 
                  ...
                  className={`flex flex-col sm:flex-row sm:items-center justify-between py-4 group ${
                    index !== commits.length - 1 ? 'border-b border-white/10' : ''
                  }`}
                >
                  <div className="flex items-center space-x-4 mb-2 sm:mb-0">
                    <span className="font-mono text-[13px] text-white/60 group-hover:text-white/90 transition-colors">
                      {item.sha.substring(0, 7)}
                    </span>
                    <span className="text-[15px] text-white group-hover:text-white/80 transition-colors font-medium">
                      {truncate(item.commit.message.split('\n')[0], 72)}
                    </span>
                  </div>
                  <span className="text-[13px] text-white/60 sm:ml-4 whitespace-nowrap">
