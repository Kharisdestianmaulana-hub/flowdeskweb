...
async function run() {
  try {
    ...
    let res = await query("ALTER TABLE posts ADD COLUMN published_at DATETIME");
    console.log(JSON.stringify(res));

    console.log("Updating published_at to created_at...");
    res = await query("UPDATE posts SET published_at = created_at WHERE published_at IS NULL");
    console.log(JSON.stringify(res));
  } catch (error) {
    console.error(error);
  }
